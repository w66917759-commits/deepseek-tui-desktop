...
- npm package: `deepseek-tui@0.8.14`
- Local bundled binary after install: `node_modules/deepseek-tui/bin/downloads/deepseek`
- Verified local binary: `deepseek 0.8.14`
...
The app is packaged as an unsigned macOS arm64 DMG for local testing:

```bash
npm run dist:mac
```

Public distribution still needs Developer ID signing, notarization, and likely a universal x64/arm64 build.
...
- Skills and MCP do not appear by default. They open as a right-side inspector only when selected.
